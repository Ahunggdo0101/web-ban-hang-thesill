...
export default function LargePlantsPage() {
  useDocumentTitle('Cây trồng trong nhà cỡ lớn và khổng lồ | Nghệ Nhân Cây Cảnh Đỗ Xuân Hùng');
  ...
  // Fetch các sản phẩm cỡ lớn và cấu hình slots từ backend
  useEffect(() => {
    const loadPageData = async () => {
      setIsLoading(true);
      setError(null);
      try {
        // 1. Gọi lấy danh sách toàn bộ sản phẩm để lọc (size=large hoặc category=large-plants)
        ...
        const filteredList = (productsData.items || []).filter(p => p.size === 'large' || p.category === 'large-plants');
        
        // 2. Gọi lấy cấu hình vị trí slots của Cây cỡ lớn
        const configRes = await fetch(`${API_BASE_URL}/collection-config/large-plants`);
        let loadedSlots = [];
        ...
        console.error('Lỗi khi tải dữ liệu trang cây lớn:', err);
      ...
      <div className="max-w-[1650px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-[146px] py-16">
        ...
        <div className="text-left mb-12">
          <h1 className="font-serif text-4xl sm:text-5xl font-light leading-tight text-brand-forest mt-2">
            Cây trồng trong nhà cỡ lớn và khổng lồ
          </h1>
          <p className="text-base sm:text-lg text-brand-slate max-w-3xl mt-4 font-normal leading-relaxed">
            Được thiết kế để tạo điểm nhấn cho căn phòng, những loại cây này mang lại quy mô và cấu trúc cho không gian, với bộ rễ chắc khỏe giúp chúng dễ dàng thích nghi một cách đáng ngạc nhiên.
          </p>
        </div>
          ...
          <div className="text-center py-24 bg-brand-white border border-brand-sand max-w-md mx-auto space-y-6">
            ...
            <div className="space-y-2 px-6">
              ...
              <p className="text-xs text-[#666] max-w-xs mx-auto leading-relaxed font-semibold">
                Rất tiếc, các bộ lọc hiện tại của bạn không khớp với bất kỳ cây cỡ lớn nào.
              </p>
            </div>
            <div className="pt-2">
              <button
        ...
        <div className="mt-24 border-t border-brand-sand/30 pt-16 max-w-5xl text-left space-y-8 pb-12">
          <div className="space-y-6">
            <h2 className="font-serif text-3xl md:text-[34px] font-normal text-brand-forest tracking-wide mb-6">
              Cây trồng trong nhà cỡ lớn & Cây cảnh khổng lồ
            </h2>
            ...
            <div className="space-y-8 text-brand-charcoal text-base md:text-[16.5px] leading-[1.8]">
              ...
              <div className="space-y-2">
                <h3 className="font-serif text-lg md:text-xl lg:text-[21px] font-normal text-brand-forest mt-4">
                  Cây trồng sàn và cây xanh trong nhà cho mọi không gian.
                </h3>
                <p className="text-brand-slate/95">
                  Tạo điểm nhấn với một chậu cây lấp đầy căn phòng. Bộ sưu tập cây trồng trong nhà cỡ lớn và khổng lồ của chúng tôi bao gồm các loại cây trồng sàn và cây xanh trong nhà mang đến sự ấn tượng, kết cấu và sức sống cho phòng khách, lối vào, văn phòng và nhiều không gian khác. Không cần phải có kinh nghiệm chăm sóc cây.
                </p>
              </div>

              <div className="space-y-2">
                <h3 className="font-serif text-lg md:text-xl lg:text-[21px] font-normal text-brand-forest mt-4">
                  Các loại cây trồng trong nhà cỡ lớn phổ biến:
                </h3>
                <p className="text-brand-slate/95">
                  Từ những cây đa cao vút đến những cây Dracaena có hình dáng độc đáo, bộ sưu tập cây trồng trong nhà cỡ lớn của chúng tôi có nhiều lựa chọn cho mọi căn phòng. Cây cọ Parlor và Areca thân thiện với vật nuôi mang đến cảm giác nhiệt đới mà không cần lo lắng, trong khi cây lưỡi hổ và cây ZZ có hình dáng nổi bật và khả năng chịu đựng đặc biệt tốt với ánh sáng yếu và tưới nước không đều. Cho dù không gian hay trình độ chăm sóc cây của bạn như thế nào, luôn có một loại cây cỡ lớn phù hợp để phát triển tốt.
                </p>
              </div>

              <div className="space-y-2">
                <h3 className="font-serif text-lg md:text-xl lg:text-[21px] font-normal text-brand-forest mt-4">
                  Cây lớn có dễ chăm sóc hơn không?
                </h3>
                <p className="text-brand-slate/95">
                  Thật ngạc nhiên, câu trả lời là có. Cây cảnh trong nhà cỡ lớn thường dễ chăm sóc hơn so với những cây nhỏ hơn. Chậu lớn hơn đồng nghĩa với lượng đất nhiều hơn, giúp giữ ẩm lâu hơn, vì vậy bạn sẽ tưới nước ít thường xuyên hơn và có nhiều "đệm" hơn nếu lỡ quên. Cây lớn cũng thường phát triển tốt hơn và có sức sống mạnh mẽ hơn, khiến chúng trở thành lựa chọn tuyệt vời cho bất cứ ai muốn tạo ấn tượng mà không cần tốn nhiều công sức chăm sóc.
                </p>
              </div>

              <div className="space-y-2">
                <h3 className="font-serif text-lg md:text-xl lg:text-[21px] font-normal text-brand-forest mt-4">
                  Cây cảnh trong nhà cỡ lớn như một điểm nhấn thiết kế:
                </h3>
                <p className="text-brand-slate/95">
                  Một chậu cây lớn duy nhất có thể làm thay đổi hoàn toàn căn phòng. Cây trồng trên sàn và cây thân gỗ trong nhà tạo nên những điểm nhấn tự nhiên, định hình không gian, thêm sự ấm áp cho nội thất tối giản và mang cảm giác thiên nhiên vào trong nhà. Cho dù bạn đang trang trí nhà mới, làm mới văn phòng hay lấp đầy một góc trống, một chậu cây lớn là một trong những lựa chọn thiết kế hiệu quả nhất mà bạn có thể thực hiện.
                </p>
              </div>
              ...
              <div className="space-y-8 pt-6 border-t border-brand-sand/30">
                ...
                <div className="space-y-2">
                  <h4 className="font-serif text-lg md:text-[20px] italic font-normal text-brand-forest mt-4">
                    Những loại cây trồng trong nhà cỡ lớn nào phù hợp nhất với điều kiện ánh sáng yếu?
                  </h4>
                  <p className="text-brand-slate/95">
                    Cây lưỡi hổ (Sansevieria) và cây ZZ là hai trong số những loại cây trồng trong nhà cỡ lớn phổ biến nhất cho những không gian thiếu sáng. Cả hai đều chịu được ánh sáng gián tiếp và ánh sáng nhân tạo, đồng thời cần tưới nước tối thiểu, lý tưởng cho văn phòng, hành lang và các phòng có ánh sáng tự nhiên hạn chế.
                  </p>
                </div>

                <div className="space-y-2">
                  <h4 className="font-serif text-lg md:text-[20px] italic font-normal text-brand-forest mt-4">
                    Những loại cây cảnh trong nhà cỡ lớn nào an toàn cho thú cưng?
                  </h4>
                  <p className="text-brand-slate/95">
                    Các loại cây cảnh trong nhà cỡ lớn thân thiện với thú cưng bao gồm cây cọ cảnh (Parlor Palm), cây cau cảnh (Areca Palm) và cây ô liu.
                  </p>
                </div>

                <div className="space-y-2">
                  <h4 className="font-serif text-lg md:text-[20px] italic font-normal text-brand-forest mt-4">
                    Bạn tưới nước cho cây trồng trong nhà cỡ lớn bao lâu một lần?
                  </h4>
                  <p className="text-brand-slate/95">
                    Hầu hết các cây trồng trong nhà cỡ lớn cần được tưới nước một đến hai tuần một lần ở nơi có ánh sáng mạnh, và ít thường xuyên hơn ở nơi có ánh sáng trung bình đến yếu. Chậu và khối lượng đất càng lớn, cây càng giữ ẩm lâu hơn, do đó cây lớn có thể được tưới nước không quá thường xuyên như cây nhỏ.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-brand-slate/95">
                    Kích thước của cây trồng trong nhà rất khác nhau tùy thuộc vào từng loài. Cây lưỡi hổ thường cao từ 60 đến 1,2 mét, trong khi cây đa và cây dracaena có thể cao tới 1,8 mét hoặc hơn nếu được chăm sóc và chiếu sáng đúng cách.
                  </p>
                </div>
